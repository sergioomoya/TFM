...
def main():
    ...
    for ax, (key, label, color) in zip(axes, [
        ...
        ax.set_ylabel(label)
        ax.set_title(f'{label}\n(Undersample {UNDERSAMPLE_LEGIT_RATIO}:1)', fontsize=12)
        ax.set_xticks(x_pos)
        ax.set_xticklabels(model_names, rotation=15, ha='right')
        ax.set_ylim([0, 1.05])
    fig.suptitle(f'Validación prequential ({n_folds} folds) — Submuestreo legítimas {UNDERSAMPLE_LEGIT_RATIO}:1', fontsize=14)
    plt.tight_layout()
    ...
    fig_cm.suptitle(f'Matrices de Confusión — Undersample {UNDERSAMPLE_LEGIT_RATIO}:1 ({n_folds} folds, threshold=0.5)', fontsize=13)
    plt.tight_layout()
