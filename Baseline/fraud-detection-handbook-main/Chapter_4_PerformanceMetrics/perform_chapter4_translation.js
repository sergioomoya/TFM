...
function translateNotebook(filePath, replacements) {
    ...
    try {
        ...
        nb.cells.forEach(cell => {
            ...
            cell.source.forEach(line => {
                ...
                for (const [en, es] of Object.entries(replacements)) {
                    if (newLine.includes(en)) {
                        newLine = newLine.replace(en, es);
                        changed = true;
                    }
                }
                newSource.push(newLine);
            });
        ...
        if (changed) {
            ...
            fs.writeFileSync(filePath, JSON.stringify(nb, null, 4), 'utf-8');
        } else {
...
// Use relative paths assuming we run from the directory containing the notebooks
const assessmentFile = "Assessment_RealWorldData.ipynb";
const topkFile = "TopKBased.ipynb";

translateNotebook(assessmentFile, assessmentReplacements);
translateNotebook(topkFile, topkReplacements);
